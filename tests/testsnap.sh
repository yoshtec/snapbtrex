...
test_local_sync(){
  for i in {1..20}
  do
    snapbtrex --path "$SNAPSHOT" --snap "$SUBVOLUME" --target-backups 10 --verbose --sync-target "./$LMNT/.sync/" --sync-keep 5
    test_equal "$?" 0 "Run: $i"
    sleep 1
  done
...
test_local_latest(){
  for i in {1..5}
  do
    snapbtrex --path "$SNAPSHOT" --snap "$SUBVOLUME" --target-backups 10 --keep-only-latest --verbose
    test_equal "$?" 0 "Run: $i"
    sleep 1
  done
...
test_local_size(){
  ...
  do
    ...
    snapbtrex --path "$SNAPSHOT" --snap "$SUBVOLUME" --verbose --target-freespace 15M --keep-backups 1
